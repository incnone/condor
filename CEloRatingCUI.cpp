                                  ...
                                  std::ostream &out)
{
 ...
 switch (ArrayLookup(pszCommand, tszCommands))
 {
  case IDC_Help: ///////////////////////////////////////////////////////////
   ...
   out << "printcovariance . print the covariance matrix";
   out << '\n';
  break;
...
//   btRes.MinorizationMaximization(0, 0);
  ...
  case IDC_PrintCovariance: //////////////////////////////////////////////////////
  {
   ...
   /*
   int First = 0;
   int Players = mCovariance.GetRows();
   int Width = 3;
   std::istringstream(pszParameters) >> First >> Players >> Width;
   if (Width < 2)
    Width = 2;
   if (First < 0)
    First = 0;
   if (First + Players > mCovariance.GetRows())
    Players = mCovariance.GetRows() - First;
   if (Players <= 0)
   {
    First = 0;
    Players = mCovariance.GetRows();
   }
   */
   int First = 0;
   int Players = mCovariance.GetRows() - First;
   int Width = 8;
   const double Mult = std::pow(10.0, (Width > 8 ? 8 : Width) - 1);
   ...
   */
   ...
   for (int p = First; p < First + Players; p++)
   {
    ...
    {
      int j = vPermutation[q];
      //out << std::setw(Width - 1) << int(mCovariance.GetElement(i, j) /* * Mult */) << ' ';
      out << int(mCovariance.GetElement(i, j)) << '\t';
    }
    out << '\n';
   }
  }
